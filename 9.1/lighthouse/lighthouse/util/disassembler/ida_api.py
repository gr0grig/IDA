...
class IDACoreAPI(DisassemblerCoreAPI):
    ...
    def _get_ida_bg_color_from_file(self):
        ...
        IDA 9.x compatibility:
          - ida_loader.gen_file expects FILE*
          - idaapi.eclose removed
          - use ida_diskio.qfopen/qfclose
        """
        ...
        ida_fd = None
        try:
            # Prefer modern disk I/O API (IDA 7.4+ / 8+ / 9+)
            try:
                import ida_diskio
            except Exception:
                ida_diskio = None

            # Open FILE* for writing text/binary; gen_file writes bytes anyway
            if ida_diskio and hasattr(ida_diskio, "qfopen"):
                ida_fd = ida_diskio.qfopen(path, "w")
            elif hasattr(idaapi, "qfopen"):
                ida_fd = idaapi.qfopen(path, "w")
            elif hasattr(idaapi, "fopenWT"):
                # legacy (older IDA)
                ida_fd = idaapi.fopenWT(path)
            else:
                logger.debug(" - No supported file-open API found for HTML export")
                return None

            if not ida_fd:
                logger.debug(" - Failed to open temp file for HTML export")
                return None

            # Generate HTML listing to FILE*
            try:
                import ida_loader
                gen_file_fn = ida_loader.gen_file
            except Exception:
                gen_file_fn = idaapi.gen_file
            ...
            gen_file_fn(
                ...
                imagebase,
                imagebase + 0x20,
                idaapi.GENFLG_GENHTML
            )

        except Exception:
            logger.exception("Failed to extract HTML for theme detection")
            return None
            ...
            try:
                if ida_fd:
                    if "ida_diskio" in locals() and ida_diskio and hasattr(ida_diskio, "qfclose"):
                        ida_diskio.qfclose(ida_fd)
                    elif hasattr(idaapi, "qfclose"):
                        idaapi.qfclose(ida_fd)
                    elif hasattr(idaapi, "eclose"):
        ...
        # Parse bgcolor from HTML variants
        bg_color_text = get_string_between(html, '<body bgcolor="', '">')
        if bg_color_text:
            logger.debug(" - Extracted bgcolor '%s' from html body" % bg_color_text)
